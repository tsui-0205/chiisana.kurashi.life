...
export default function EditPost() {
    ...
    const loadPost = async () => {
        try {
            ...
            if (data.posts) {
                const post = data.posts.find(p => p.id === decodeURIComponent(postId));
                if (post) {
                    setFormData({
                        title: post.title || "",
                        slug: post.id || "",
    ...
    const handleChange = (e) => {
        ...
        // タイトルが変更されたら自動でslugを生成（編集時はslugは変更しない）
    ...
    const confirmSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                showToast('記事が正常に更新されました！', 'success');
                setTimeout(() => {
                    router.push('/site/admin');
                }, 1000);
            } else {
                ...
                {!preview ? (
                    ...
                    <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                        ...
                        <div className="text-center py-8">
                            <p className="text-gray-600">プレビュー機能（簡略化）</p>
                            <h2 className="text-2xl font-bold mt-4">{formData.title}</h2>
                            <p className="text-gray-600 mt-2">{formData.category}</p>
                        </div>
                    </div>
                )}
            </div>
